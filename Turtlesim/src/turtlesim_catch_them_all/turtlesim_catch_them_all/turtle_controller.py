...
# subscribing Pose , publishing cmd_vel

class TurtlecontrollerNode(Node): # MODIFY NAME

    def __init__(self):
        super().__init__("turtle_controller")
        # we need to catch the turtles
    ...
    def callback_alive_turtles(self,msg):
        # controller has to know about turtles so we need to subscribe from turtle_spawner
    ...
    def control_loop(self):
        # we don't do anything in control loop unless we have a pose that came from thae total one poste
        ...
        msg = Twist()
        # need to send linear and angular speed

        if distance > 0.5:
            # target is not reached
            # PID controller is to be used

            # position
            msg.linear.x = 2*distance # multiplying by 2 makes P controller better
            ...
            if diff > math.pi:
                ...
                diff += 2*math.pi
            msg.angular.z = 6*diff # helps in PID controller
            ...
            self.turtle_to_catch_ = None
            # when we reached targetit's useless to continue to run the control loop
